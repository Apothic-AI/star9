...
fn live_http_backend_exercises_real_server_edges_when_configured() -> Result<()> {
    if !live_enabled("STAR9_LIVE_HTTP") {
        eprintln!("skipping live HTTP backend test; set STAR9_LIVE_HTTP=1");
        return Ok(());
    }
    let Some(base_url) = opt_env("STAR9_LIVE_HTTP_BASE_URL") else {
        eprintln!("skipping live HTTP backend test; STAR9_LIVE_HTTP_BASE_URL is unset");
        return Ok(());
    };
    let transport = NativeHttpTransport::new();

    let mut headers = BTreeMap::new();
    if let Some(auth) = opt_env("STAR9_LIVE_HTTP_AUTH") {
    ...
    if let Some(range_url) = opt_env("STAR9_LIVE_HTTP_RANGE_URL") {
    ...
    if let Some(redirect_url) = opt_env("STAR9_LIVE_HTTP_REDIRECT_URL") {
    ...
    if let Some(auth_failure_url) = opt_env("STAR9_LIVE_HTTP_AUTH_FAILURE_URL") {
...
fn live_s3_r2_bucket_exercises_object_store_edges_when_configured() -> Result<()> {
    if !live_enabled("STAR9_LIVE_S3") {
        eprintln!("skipping live S3/R2 bucket test; set STAR9_LIVE_S3=1");
        return Ok(());
    }
    let Some(endpoint) = opt_env("STAR9_S3_ENDPOINT") else {
        eprintln!("skipping live S3/R2 bucket test; STAR9_S3_ENDPOINT is unset");
        return Ok(());
    };
    let Some(bucket) = opt_env("STAR9_S3_BUCKET") else {
        eprintln!("skipping live S3/R2 bucket test; STAR9_S3_BUCKET is unset");
        return Ok(());
    };
    let Some(access_key_id) = opt_env("STAR9_S3_ACCESS_KEY_ID") else {
        eprintln!("skipping live S3/R2 bucket test; STAR9_S3_ACCESS_KEY_ID is unset");
        return Ok(());
    };
    let Some(secret_access_key) = opt_env("STAR9_S3_SECRET_ACCESS_KEY") else {
        eprintln!("skipping live S3/R2 bucket test; STAR9_S3_SECRET_ACCESS_KEY is unset");
        return Ok(());
    };
    let region = opt_env("STAR9_S3_REGION").unwrap_or_else(|| "auto".to_string());
    let service = opt_env("STAR9_S3_SERVICE").unwrap_or_else(|| "s3".to_string());
    let prefix = opt_env("STAR9_S3_PREFIX").unwrap_or_else(default_live_prefix);
    ...
    metadata.insert("x-amz-meta-star9-live".to_string(), "true".to_string());
    store.put(
        "object-a.txt",
    ...
    assert!(
        header(&object.metadata, "x-amz-meta-star9-live").is_some()
            || header(
                &object.metadata,
                "x-amz-meta-star9-live".trim_start_matches("x-amz-meta-")
            )
            .is_some(),
        "live object metadata did not include star9 marker: {:?}",
        object.metadata
    );
    ...
    if live_enabled("STAR9_LIVE_S3_AUTH_FAILURE") {
        let bad_signer = AwsSigV4Signer::new("STAR9_BAD_ACCESS_KEY", "STAR9_BAD_SECRET", "auto");
        let bad_store = S3ObjectStore::with_signer(
            opt_env("STAR9_S3_ENDPOINT").unwrap(),
            opt_env("STAR9_S3_BUCKET").unwrap(),
            Some(prefix.clone()),
            Arc::new(NativeHttpTransport::new()),
            Arc::new(bad_signer),
        );
