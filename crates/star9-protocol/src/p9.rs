//! Rust-native 9P2000.L subset used for Star 9 import/export bridges.
//!
...
use star9_core::{base_name, clean_path, parent_path, valid_path};
use star9_fs::{
...
mod tests {
    use super::*;
    use star9_fs::{directory_file, fs_ref, MemFs};
    ...
    fn raw_server_lists_xattrs_via_empty_xattrwalk_name() {
        let mem = MemFs::from_entries([("dir/file.txt", b"hello".as_slice())]);
        fs::set_xattr(&mem, "dir/file.txt", "user.author", b"star9").unwrap();
        fs::set_xattr(&mem, "dir/file.txt", "user.mime_type", b"text/plain").unwrap();
        let server = NinePServer::new(fs_ref(mem));
        server
            .handle_frame(
    ...
    fn client_filesystem_round_trips_xattrs() {
        ...
        fs::set_xattr(&client, "dir/file.txt", "user.author", b"star9").unwrap();
        assert_eq!(
            fs::list_xattrs(&client, "dir/file.txt").unwrap(),
