...
use star9_core::{Error, ErrorKind, FileMode, OpenFlags, Result};
use star9_fs::{self as fs, FileSystem};
use star9_task::Task;
...
impl From<star9_core::Metadata> for StatInfo {
    fn from(meta: star9_core::Metadata) -> Self {
...
impl Star9Api {
    ...
    pub fn open(&self, path: &str) -> Result<u32> {
        let file = self
            .task
            .namespace()
            .open(&star9_core::FsContext::new(), path)?;
...
mod tests {
    ...
    use star9_task::TaskFs;
    use star9_vfs::BindMode;

    fn api_with_memfs() -> Star9Api {
