...
use wanix_fs::{directory_file, FileSystem, FsRef};
...
impl FileSystem for Namespace {
    fn open(&self, ctx: &FsContext, name: &str) -> Result<wanix_fs::BoxFile> {
        if !valid_path(name) {
    ...
    fn create(&self, name: &str) -> Result<wanix_fs::BoxFile> {
