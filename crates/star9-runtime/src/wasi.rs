...
use star9_core::{clean_path, Error, ErrorKind, FileMode, FsContext, OpenFlags, Result};
use star9_fs::{read_file, FileHandle, FileSystem};
use star9_protocol::runtime::{EnvironmentEntry, ExecutionSpec, ExitStatus};
use star9_task::Task;
...
fn add_wasi_imports(linker: &mut Linker<WasiState>) -> Result<()> {
    ...
    linker
        ...
        .map_err(wasmi_error)?;
    linker
        .func_wrap(WASI, "sock_accept", sock_accept)
        .map_err(wasmi_error)?;
    linker
        .func_wrap(WASI, "sock_recv", sock_recv)
        .map_err(wasmi_error)?;
    linker
        .func_wrap(WASI, "sock_send", sock_send)
        .map_err(wasmi_error)?;
    linker
        .func_wrap(WASI, "sock_shutdown", sock_shutdown)
        .map_err(wasmi_error)?;
    linker
...
mod tests {
    use super::*;
    use star9_fs::{fs_ref, open, read_file, MemFs};
