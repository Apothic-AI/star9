//! Star9 task/resource filesystem.
...
mod tests {
    use super::*;
    use star9_fs::{open, read_file, MemFs};
    ...
    fn fd_lifecycle_routes_to_open_file() {
        ...
        let fd = task.open_fd(star9_fs::open(&mem, "file").unwrap(), "file");
        let mut buf = [0_u8; 2];
    ...
    fn renumber_fd_moves_source_into_existing_target() {
        ...
        let source = task.open_fd(star9_fs::open(&mem, "source").unwrap(), "source");
        let target = task.open_fd(star9_fs::open(&mem, "target").unwrap(), "target");
    ...
    fn explicit_fd_setup_supports_standard_streams() {
        let task = TaskFs::new().alloc("auto", None).unwrap();
        let stdin = star9_fs::Node::file("stdin", b"input".to_vec(), FileMode::from_perm(0o666));
        ...
        let next = task.open_fd(
            star9_fs::Node::file("next", Vec::new(), FileMode::from_perm(0o666))
                .open(&FsContext::new(), ".")
                .unwrap(),
            "next",
        );
    ...
    fn task_namespace_clones_parent_bindings() {
        ...
        let mem = star9_fs::fs_ref(MemFs::from_entries([("file", b"value".to_vec())]));
        parent
            .namespace()
        ...
        assert_eq!(
            star9_fs::read_file(child.namespace().as_ref(), "mnt/file").unwrap(),
            b"value"
        );
    }
    ...
    fn task_export_is_optional_and_routes_filesystem_access() {
        let fs = TaskFs::new();
        let task = fs.alloc("auto", None).unwrap();
        assert!(star9_fs::stat(&task, "export").is_err());

        let export = star9_fs::fs_ref(MemFs::from_entries([
        ...
        assert_eq!(
            star9_fs::read_dir(&task, "export/dir")
                .unwrap()
                .into_iter()
