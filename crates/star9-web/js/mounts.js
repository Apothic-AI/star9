...
import { createStar9P9NamespaceMount } from "./p9-port.js";
...
export async function createP9MountFromPort(port, options = {}) {
    return createStar9P9NamespaceMount(port, options);
}
...
export function normalizeMountPath(path) {
    ...
    if (value.startsWith("/") || value.includes("\\")) {
        throw new Error(`mount paths must be relative Star9 paths: ${JSON.stringify(path)}`);
    }
    const parts = [];
