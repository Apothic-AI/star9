...
export class BindElement extends HTMLElement {
    ...
    toBinding() {
        ...
        if ((kind === "archive" || kind === "import") && !src && !storage) {
            throw new Error(`star9-bind kind=${JSON.stringify(kind)} requires a src attribute`);
        }
...
function normalizeKind(kind) {
    ...
    throw new Error(`Unsupported star9-bind kind ${JSON.stringify(kind)}`);
}
...
function parseStorageDescriptor(element) {
    ...
    try {
        ...
        throw new Error(`Invalid star9-bind storage JSON: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
function requireAttribute(element, name) {
    ...
    if (!value) {
        throw new Error(`star9-bind is missing required ${name} attribute`);
    }
    return value;
}
