...
export const DEFAULT_RUNTIME_PORT_MESSAGE_TYPE = "star9-worker-runtime";
...
export async function resolveStar9SystemFacade(systemLike) {
...
export async function connectRuntimePort(target, options = {}) {
    ...
    const system =
        options.system == null ? { element: null, facade: null } : await resolveStar9SystemFacade(options.system);
...
export function wireRuntimeEndpointToSystem(endpoint, facade, options = {}) {
    ...
    if (!facade || typeof facade.handleRuntimeRequest !== "function") {
        throw new TypeError("expected a Star9System facade with handleRuntimeRequest(bytes)");
    }
...
export function requestStar9ImportPort(src, options = {}) {
    if (typeof document === "undefined") {
        throw new Error("requestStar9ImportPort requires a browser document");
    }

    const targetOrigin = options.targetOrigin || "*";
    const request = options.request || "star9-import";
        ...
        iframe.onload = () => {
            if (timeoutMs >= 0) {
                timeoutTimer = setTimeout(() => {
                    rejectOnce(new Error(`timed out waiting for Star9 import port from ${JSON.stringify(url)}`));
                }, timeoutMs);
            }
            sendRequest();
            if (retryMs > 0) {
...
export function createStar9RuntimeNamespaceClient(endpoint, options = {}) {
            ...
            const timer =
                timeoutMs >= 0
                    ? setTimeout(() => {
                        ...
                        reject(new Error(`timed out waiting for Star9 runtime ${method} response`));
                    }, timeoutMs)
                    : null;
    ...
    return {
        ...
        close() {
            ...
            while (pending.length > 0) {
                const next = pending.shift();
                clearTimeout(next.timer);
                next.reject(new Error("Star9 runtime namespace client closed"));
            }
        },
    };
}
