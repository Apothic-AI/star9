...
- Wasmi-backed WASI preview1 execution coverage for task namespace module loading, args/env/cwd propagation, preopened cwd, fd read/write/pread/pwrite/seek/tell/allocate/close/stat/sync/datasync/truncate/readdir behavior, path open/stat/create-directory/unlink/remove-directory/rename/symlink/readlink behavior, stdio fd writes, deterministic random/clock imports, preview1 errno mapping, and proc-exit mapping.
...
- WASI syscall coverage beyond the current preview1 fd directory, fd positional I/O/allocation, fd sync/truncate, path mutation, args/env, clock, and random baseline.
