...
`docs/audits/rc-process-graph-matrix.json` and `docs/audits/rc-pipeline-job-control-boundary.md` record the rc process-graph boundary. The portable rc evaluator remains deterministic, while the Star 9 adapter creates task/fd/pipe graph records for pipelines, background jobs, and process substitution. Native-host WASI, registered JS/WASM, and opt-in native rc pipelines/background jobs now use provider-backed task/fd execution with persistent `.rc/graphs/<id>` lifecycle and `ctl` files, deterministic active-job note status, and file redirect descriptors. Browser worker graph-compatible pipelines/jobs use a browser provider with bounded stdin/stdout handoff. Star 9 rc parses Plan 9 `rfork nNeEsfFm` flags with default `ens` and maps them to task namespace/env/fd/note/no-mount scopes where the runtime can expose exact file-visible behavior. Provider-specific hard cancellation for handlers without interrupts and arbitrary nested fd graph concurrency remain documented provider boundaries.
