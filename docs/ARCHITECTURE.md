...
`star9-cli shell` uses `reedline` for interactive input and keeps host process execution explicit through `--native` plus the `native <cmd...>` shell command. The CLI shell is rc-first: `star9 shell` and `star9 rc` both run through the reusable rc core and Star 9 adapter, while `star9 shell --simple` selects the smaller admin parser. Browser shell support is exposed as `Star9System.createShell()`, `Star9System.createRcShell()`, `crates/star9-web/js/shell.js`, and the `<star9-shell>` custom element. `<star9-shell>` is rc-first by default, and `<star9-shell simple>` selects the small admin parser. Browser-only helpers such as `mount-opfs`, `mount-starfs`, `import`, `srv import!url#system`, `srv ws!host!path`, `srv wss!host!path`, `srv webtransport!host!path`, and `download` call the existing mounted-storage/import/service/facade paths rather than bypassing the namespace model.
