...
The JS storage modules under `crates/wanix-web/js/` provide browser-native host adapters for OPFS/File System Access, Cache API, DOM, download, JS value, and worker-backed storage. They are async host adapters with deterministic fake-host tests. The `mounts.js` async mount table gives `wanix-system` a browser-side namespace overlay for these adapters when real browser APIs cannot satisfy the synchronous Rust `FileSystem` trait directly, and its browser debounced sync scheduler provides `setTimeout`-backed scheduling for async storage/sync targets. Native Rust tests still use descriptor-backed `BrowserStorageRegistry` stand-ins where a synchronous `FileSystem` is required.

`wanix-web` also provides a host-neutral `MessagePort` trait, an in-memory port for native tests, and worker-runtime adapters that move typed runtime requests, responses, and task messages over complete message payloads. The JS `worker-runtime.js` module adds browser-native Worker/MessagePort helpers for tagged binary envelopes, endpoint listeners, transferred ports, system facade resolution, import-port requests, and optional bridging of runtime request/task-message payloads into a `WanixSystem` facade. Cross-document import requests retry until the responder is ready and keep the exporting iframe alive until the imported port is closed. The JS `p9-port.js` module serves complete 9P frames over browser MessagePorts, offers a tag-matched async frame client with AbortSignal-driven `Tflush`, provides an async 9P namespace mount client for browser read/write/list operations, cancels Promise-backed server work on `Tflush`, suppresses late replies, and answers `wanix-import` requests by transferring a served port backed by the Rust `WanixSystem::handle9pFrame` facade. The JS `worker-host.js` module builds on the runtime layer to spawn or attach Worker-like targets, transfer a runtime port, surface request/response/task-message listeners plus ordinary worker messages, and manage stop/restart/cleanup with fake-host coverage. Ordinary worker messages carrying `{ export: MessagePort }` are interpreted by `wanix-system` as 9P exports: the port is mounted at `#task/<id>/export`, and a `vm` field also mounts the same export at `#vm/<id>/guest`. The JS `js-wasm-worker-host.js` module layers a stable JS/WASM execution bootstrap over the same worker host so real execution shims can receive module, args, env, cwd, fd, stdio, port, task, worker, and runtime descriptor data through one message shape. The JS `js-wasm-execution-worker.js` module is the worker-side acceptor for that shape: it waits for the runtime port and execution bootstrap, builds a normalized runner context, supports dynamic JS runner imports and direct WASI-style `.wasm` instantiation, and reports deterministic task exit/error messages. The browser smoke path now exercises this stack with real module Workers, worker-exported 9P mounts, and JS/direct WASM exits through the Rust task filesystem.
