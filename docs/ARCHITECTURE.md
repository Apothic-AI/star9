...
`star9` is the primary Rust implementation of the Star 9 runtime. The legacy upstream reference checkout remains the behavior reference, but runtime behavior is delivered entirely by this repository's Rust code, specs, fixtures, and conformance tests.

## Core Values

`star9-core` owns stable value types that are shared by every other crate:

- `Error` and `ErrorKind` preserve operation/path-aware failures.
- `FileMode`, `Metadata`, and `DirEntry` model Star 9 filesystem metadata and type bits.
...
- Path helpers validate and normalize relative Star 9 paths.

## Filesystems

`star9-fs` defines `FileSystem` and `FileHandle`. Backends implement the trait directly while shared helpers provide package-level behavior for common Star 9 filesystem operations:
...
- Destination-only unmount plus source-specific `unmount src dst` behavior for removing one layer from a bind stack without requiring callers to hold filesystem object identities.

## Tasks

`star9-task` ports the task/resource filesystem:
...
`star9-protocol` defines typed requests and responses for the public Star 9 file API:
...
`Star9Api` executes those requests against a `Task`.

Typed requests and responses can be encoded and decoded as CBOR through `star9-protocol`, keeping the wire boundary Rust-owned while matching the public Star 9 operation set.

`star9-protocol::p9` provides the Rust-native 9P bridge baseline. It owns a 9P2000.L-style frame codec, a `NinePServer` that exports any `star9-fs::FileSystem`, a synchronous `NinePTransport` trait, a native `StreamTransport`/`TcpStreamTransport`, an async-capable `AsyncNinePServer` for cancellable concurrent native work, and a `NinePClientFs` that imports a remote 9P export back into the normal filesystem trait surface. The bridge includes xattr walk/create support over the existing filesystem xattr trait methods. Browser MessagePort/WebSocket adapters can wrap the frame transport without changing the core protocol implementation.

`star9-protocol::runtime` owns typed worker/task messages for spawn/start requests, execution specs, stdio/fd descriptors, port open and handoff, task message payloads, exit status, and worker-side path/fd requests for task namespace I/O. The protocol has Rust-owned CBOR round-trip tests and is the shared contract for runtime drivers and browser worker adapters.

`star9-runtime` exposes helpers to export a task namespace as 9P and import a `NinePTransport` into the root namespace. `star9-web` layers browser-facing frame helpers and smoke facade methods over those hooks while keeping the core bridge host-neutral.

`star9-runtime::RuntimeProtocolHost` handles the typed runtime protocol in a host-neutral way. It allocates worker tasks, applies execution specs to task state and descriptors, tracks in-memory port open/handoff records, records task messages, routes stdio/fd task-message bytes through installed task descriptors, and updates task exit state from exit messages. It exposes immutable snapshots for workers, ports, handoff targets, and task messages so browser glue, CLI acceptance, and tests can inspect lifecycle state without reaching into locks. The `Star9System` wasm facade exposes CBOR request and task-message handlers plus direct worker lifecycle helpers so browser `MessagePort` endpoints and real Worker hosts can route runtime traffic into this host without interpreting the Rust protocol in JavaScript. Broader native execution drivers attach beyond that contract.

## Runtime And Web

`star9-runtime` builds the root task and binds the built-in surfaces:

- `#star9` for version metadata.
- `#task` for task allocation and lookup.
- `#env` for rc/userland environment entries, plus visible `env`.
- `#srv` for named Star 9 service endpoints, plus visible `srv`, `n`, and `mnt` compatibility paths for rc/userland scripts.
...
The `EnvRegistry` behind `#env` stores userland environment files. Rc list variables are represented as NUL-separated byte entries, and exported functions use `fn#name` entries. The Star 9 rc adapter imports and exports through this file surface so environment behavior remains observable through the namespace.

The `ServiceRegistry` behind `#srv` stores Star 9-owned service descriptors rather than raw host handles. The default implementation can register a loopback 9P export of the current namespace, expose it as a file under `#srv`/`srv`, and mount it back into the namespace with ordinary `Namespace::bind` semantics. Native hosts can also register `tcp!host!port` 9P services through `TcpStreamTransport` when explicitly used. Commands such as `srv` and `mount` are userland frontends over this registry; browser service providers use honest browser address families such as `import!url#system`, `ws!`, `wss!`, or `webtransport!` rather than raw TCP.
...
`star9-shell` is the host-neutral shell layer. It deliberately starts as a Star 9 command surface rather than a POSIX or Bash compatibility layer. The parser handles words, single and double quotes, backslash escapes, comments, and `;` sequencing. `ShellSession` owns cwd, prompt state, last status, and command dispatch. `ShellHost` is the boundary that turns shell commands into Star 9 operations: file reads/writes/listing/stat, task execution, task inspection, namespace bind/unmount, service registration/mounting, and device access all go through namespaces, task files, fd files, device files, or runtime requests.

`star9-rc` is the reusable Plan 9 rc language crate. It owns rc lexing/parsing, typed AST nodes, list-valued variables, expansion, glob/pattern matching, status semantics, functions, control flow, fd redirection/duplication, process substitution, here documents, command substitution, environment export/import, note hooks, `$path` script dispatch, and evaluator behavior behind an `RcHost` trait. It intentionally does not depend on `star9-runtime`, `star9-web`, `star9-cli`, browser APIs, or native process execution.

`star9-shell::rc` is the Star 9 adapter for that reusable rc core. It implements `RcHost` over Star 9 namespace reads/writes/listing/stat, current-directory tracking, redirection targets, globbing, device-file access, environment import/export through `#env`, process-graph prepare/finish hooks over `#task` and generated pipe resources, and command execution through the existing Star 9 shell/runtime command surface. Native-host executable graph hooks can run WASI stages, registered JS/WASM stages, and opt-in native stages concurrently through `ExecutionRegistry`, task fd descriptors, generated pipe resources, and append-aware file descriptors; pure built-ins and unsupported providers retain the deterministic evaluator path. Provider-backed graphs expose `.rc/graphs/<id>/kind`, `job`, `tasks`, `notes`, `state`, `status`, and `ctl`; writing to `ctl` is the file-protocol surface for cleanup and cancellation marking. Star 9 rc maps Plan 9 `rfork nNeEsfFm` onto task namespace copy/clean scopes, env registry copy/clean scopes, task fd copy/clean scopes, rc note-group scope, and no-mount/no-`#` path enforcement. Rc scripts found through `$path` run inside the reusable evaluator; namespace `.wasm`/`.wat` command files dispatch through `wasi`, and `.js`/`.mjs` files dispatch through `worker`. Shell-visible Plan 9 commands such as `bind`, source-specific `unmount`, `srv`, and `mount` therefore become available to rc scripts without giving rc a privileged side channel. This keeps rc language compatibility separate from the Star 9 host integration and keeps Plan 9 composition intact.
...
`star9-cli shell` uses `reedline` for interactive input and keeps host process execution explicit through `--native` plus the `native <cmd...>` shell command. The CLI shell is rc-first: `star9 shell` and `star9 rc` both run through the reusable rc core and Star 9 adapter, while `star9 shell --simple` selects the smaller admin parser. Browser shell support is exposed as `Star9System.createShell()`, `Star9System.createRcShell()`, `crates/star9-web/js/shell.js`, and the `<star9-shell>` custom element. `<star9-shell>` is rc-first by default, and `<star9-shell simple>` selects the small admin parser. Browser rc uses the wasm rc evaluator for pure language behavior and a JS-side graph provider for worker-backed graph-compatible stages, so `worker module.mjs | worker other.mjs` and `worker module.mjs & wait` still flow through Star 9 browser Worker tasks and task exits. Browser-only helpers such as `mount-opfs`, `mount-starfs`, `import`, `srv import!url#system`, `srv ws!host!path`, `srv wss!host!path`, `srv webtransport!host!path`, and `download` call the existing mounted-storage/import/service/facade paths rather than bypassing the namespace model.
...
`star9-web` also provides a host-neutral `MessagePort` trait, an in-memory port for native tests, and worker-runtime adapters that move typed runtime requests, responses, and task messages over complete message payloads. The JS `worker-runtime.js` module adds browser-native Worker/MessagePort helpers for tagged binary envelopes, endpoint listeners, transferred ports, system facade resolution, import-port requests, optional bridging of runtime request/task-message payloads into a `Star9System` facade, and `createStar9RuntimeNamespaceClient(...)` for worker-side path/fd requests over the same runtime port. Cross-document import requests retry until the responder is ready and keep the exporting iframe alive until the imported port is closed. The JS `p9-port.js` module serves complete 9P frames over browser MessagePorts, offers a tag-matched async frame client with AbortSignal-driven `Tflush`, provides an async 9P namespace mount client for browser read/write/list operations, cancels Promise-backed server work on `Tflush`, suppresses late replies, rejects malformed frames, and answers origin-gated `star9-import` requests by transferring a served port backed by the Rust `Star9System::handle9pFrame` facade. The JS `network-adapter.js` module gives browser WebSocket-like transports a Plan 9-shaped network resource surface. The JS `worker-host.js` module builds on the runtime layer to spawn or attach Worker-like targets, transfer a runtime port, surface request/response/task-message listeners plus ordinary worker messages, and manage stop/restart/cleanup with fake-host coverage. Ordinary worker messages carrying `{ export: MessagePort }` are interpreted by `star9-system` as 9P exports: the port is mounted at `#task/<id>/export`, and a `vm` field also mounts the same export at `#vm/<id>/guest`. The JS `js-wasm-worker-host.js` module layers a stable JS/WASM execution bootstrap over the same worker host so real execution shims can receive module, args, env, cwd, fd, stdio, port, task, worker, and runtime descriptor data through one message shape. The JS `js-wasm-execution-worker.js` module is the worker-side acceptor for that shape: it waits for the runtime port and execution bootstrap, builds a normalized runner context, supports dynamic JS runner imports, direct WASI-style `.wasm` instantiation, and Go-compatible runner fixtures, and reports deterministic task exit/error messages. The browser smoke path now exercises this stack with real module Workers, worker-exported 9P mounts, JS/direct WASM/Go-compatible exits, OPFS task-facing storage, and StarFS storage through the Rust task filesystem boundary where applicable.

`star9-terminal` is the browser custom element wrapper for the terminal device protocol. It allocates or attaches to a `#term/<id>` resource, routes data/program/raw/resize/control operations through the same files exposed to native callers, mirrors retained screen text into the element, and emits ready/change events for browser tests and examples.

`BrowserBindingRegistry` is the host-neutral source registry for `<star9-bind>`-style file, archive, and import bindings. It maps source identifiers to bytes or 9P transports so native tests can validate file writes, tar mounts, and 9P imports. Browser custom elements register fetched file/archive bytes through the wasm facade before applying typed namespace descriptors.

`star9-runtime::ExecutionRegistry` is the host-neutral execution contract for WASI, JS-WASM, and opt-in native modules. Callers register native handlers by execution kind or module name; the registry can report whether a handler exists, applies the typed execution spec to task state and descriptors, invokes the handler, and records the returned exit status. The rc Star 9 adapter uses that contract for provider-backed process graphs: WASI, registered JS/WASM, and opt-in native stages receive task fd descriptors and pipe endpoints under `.rc/graphs/<id>`, with lifecycle state recorded in graph files. `WasmiWasiHandler` is the first real engine-backed handler: it loads a WASI module from the task namespace, preopens the task cwd, maps args/env and fd descriptors into preview1 syscalls, supports fd read/write/pread/pwrite/seek/tell/allocate/renumber/stat/advice/flags/rights/timestamps/sync/truncate/readdir, clock resolution/time, poll/yield/signal imports, deterministic socket send/recv/shutdown over installed task fds, explicit unsupported socket accept on non-listener fds, and path open/stat/timestamps/create/remove/rename/link/symlink/readlink operations over the task namespace, and returns `proc_exit` status through the normal task lifecycle. `NativePtyExecutionHandler` is available on native targets for explicitly registered host-process execution through a pseudo terminal; it routes PTY output into task stdout and records numeric exit or signal status.
