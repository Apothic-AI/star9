...
`star9-shell::rc` is the Star 9 adapter for that reusable rc core. It implements `RcHost` over Star 9 namespace reads/writes/listing/stat, current-directory tracking, redirection targets, globbing, device-file access, environment import/export through `#env`, and command execution through the existing Star 9 shell/runtime command surface. Rc scripts found through `$path` run inside the reusable evaluator; namespace `.wasm`/`.wat` command files dispatch through `wasi`, and `.js`/`.mjs` files dispatch through `worker`. Shell-visible Plan 9 commands such as `bind`, source-specific `unmount`, `srv`, and `mount` therefore become available to rc scripts without giving rc a privileged side channel. This keeps rc language compatibility separate from the Star 9 host integration and keeps Plan 9 composition intact.
