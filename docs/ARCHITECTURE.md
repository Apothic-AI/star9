...
The `ServiceRegistry` behind `#srv` stores Star 9-owned service descriptors rather than raw host handles. The default implementation can register a loopback 9P export of the current namespace, expose it as a file under `#srv`/`srv`, and mount it back into the namespace with ordinary `Namespace::bind` semantics. Commands such as `srv` and `mount` are userland frontends over this registry; native/browser network service providers can be added later behind the same file/device shape.
...
`star9-shell::rc` is the Star 9 adapter for that reusable rc core. It implements `RcHost` over Star 9 namespace reads/writes/listing/stat, current-directory tracking, redirection targets, globbing, device-file access, and command execution through the existing Star 9 shell/runtime command surface. Rc scripts found through `$path` run inside the reusable evaluator; namespace `.wasm`/`.wat` command files dispatch through `wasi`, and `.js`/`.mjs` files dispatch through `worker`. Shell-visible Plan 9 commands such as `bind`, `unmount`, `srv`, and `mount` therefore become available to rc scripts without giving rc a privileged side channel. This keeps rc language compatibility separate from the Star 9 host integration and keeps Plan 9 composition intact.
...
`star9-cli shell` uses `reedline` for interactive input and keeps host process execution explicit through `--native` plus the `native <cmd...>` shell command. The CLI shell is rc-first: `star9 shell` and `star9 rc` both run through the reusable rc core and Star 9 adapter, while `star9 shell --simple` selects the smaller admin parser. Browser shell support is exposed as `Star9System.createShell()`, `Star9System.createRcShell()`, `crates/star9-web/js/shell.js`, and the `<star9-shell>` custom element. `<star9-shell>` is rc-first by default, and `<star9-shell simple>` selects the small admin parser. Browser-only helpers such as `mount-opfs`, `mount-starfs`, `import`, and `download` call the existing mounted-storage/import/facade paths rather than bypassing the namespace model.
