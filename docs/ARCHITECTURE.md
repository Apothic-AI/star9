...
`star9-rc` is the reusable Plan 9 rc language crate. It owns rc lexing/parsing, typed AST nodes, list-valued variables, expansion, glob/pattern matching, status semantics, functions, control flow, redirection, pipelines, command substitution, and evaluator behavior behind an `RcHost` trait. It intentionally does not depend on `star9-runtime`, `star9-web`, `star9-cli`, browser APIs, or native process execution.

`star9-shell::rc` is the Star 9 adapter for that reusable rc core. It implements `RcHost` over Star 9 namespace reads/writes/listing/stat, current-directory tracking, redirection targets, globbing, device-file access, and command execution through the existing Star 9 shell/runtime command surface. This keeps rc language compatibility separate from the Star 9 host integration and keeps Plan 9 composition intact.
