...
Raw OPFS is the preferred simple persistent browser filesystem. StarFS is a separate optional mount backend, OPFS-backed by default, for durable/auditable workspaces. The checked-in StarFS adapter exposes ordinary files plus `.starfs/kv`, `.starfs/toolcalls`, and `.starfs/snapshots`; full external StarFS SDK/PrimaDB inode semantics remain an opt-in worker/wasm integration boundary rather than a default dependency.
