...
- Keep the browser-side 9P import/export tranche at whole-frame `MessagePort` serving boundaries until a Rust-owned async client mount transport is designed and validated, while using Rust-owned length-prefixed stream helpers for native 9P serving/import paths.
