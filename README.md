...
- Reusable Plan 9 rc language core with a Star 9 host adapter for native/browser rc sessions, including task/fd graph records for pipelines, background jobs, process substitution, native-host provider-backed WASI/registered-JS/opt-in-native pipeline execution, browser worker graph-compatible pipelines, and `.rc/graphs/<id>/ctl` cleanup/cancel controls.
